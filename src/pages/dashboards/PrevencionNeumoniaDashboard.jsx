...
export default function PrevencionNeumoniaDashboard() {
  ...
  const summaryUb = useMemo(() => {
    ...
    rows.forEach(r => {
      ...
      if (!map[ub]) map[ub] = { ub, regs: 0, cumple: 0, total: 0, casos: 0 }
      map[ub].regs++
      map[ub].casos += r.num_casos ?? 0
      PN_KEYS.forEach(k => {
        map[ub].total++
  ...
  return (
    <div ref={pdfRef} className="p-6 lg:p-8 animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard label="Paquete Completo (4/4)" value={`${pctPaquete}%`}
          sub={`${totalPaquete} con todos los criterios`}
          color={pctPaquete >= 80 ? 'emerald' : 'amber'} />
        <KpiCard label="Total Casos"            value={totalCasos}
          sub="Suma de pacientes en los registros" color="slate" />
      </div>
            ...
            <div className="card p-5">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={criteriosData} margin={{ top: 22, left: -10 }}>
                  ...
                  <Bar dataKey="cumple" name="Cumple" fill="#8b5cf6" stackId="a" hide={tglCriterios.hidden.has('cumple')} isAnimationActive={false}>
