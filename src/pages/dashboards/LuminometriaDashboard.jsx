    ...
    <div className="card p-5">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className={`border-b border-white/70 ${i % 2 === 1 ? 'bg-white/55' : ''} hover:bg-amber-50 transition-colors`}>
    ...
    <div ref={pdfRef} className="p-6 lg:p-8 animate-fade-in space-y-6">
      ...
      <div className="card p-4" data-pdf-hide>
        ...
        <div className="filters-row" style={{ '--cols': 4 }}>
          ...
          <div>
            <label>Objeto / Superficie</label>
            <select className="input" value={filters.objeto} onChange={e => setF('objeto', e.target.value)}>
              <option value="">Todos</option>
              ...
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  <Pie data={pieData} cx="50%" cy="50%" outerRadius={90}
                    ...
                    labelLine
                    onClick={d => abrirDetalle(`Resultado — ${d.name}`, medidos.filter(r => (r.rango === 'CUMPLE' ? 'CUMPLE (<100 RLU)' : 'NO CUMPLE (≥100 RLU)') === d.name))}>
