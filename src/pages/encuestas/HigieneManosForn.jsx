...
export default function HigieneManosForn() {
  ...
  async function onSubmit(vals) {
    ...
    const { error } = await guardarEncuesta('encuesta_higiene_manos', payload, isEdit ? id : undefined)
    if (error) {
      setSaveError(error)
      setSaving(false)
      return
  ...
  if (!isEdit && !puedeCapturar) {
    return <SinPermisoCaptura volverTo="/encuestas/higiene-manos" />
  }
  ...
  return (
    <div className="p-6 lg:p-8 animate-fade-in max-w-3xl">
      <div className="flex items-start gap-3 mb-6">
        ...
        <Link to="/encuestas/higiene-manos/dashboard"
