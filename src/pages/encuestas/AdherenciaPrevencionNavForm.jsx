...
import { ArrowLeft, Save, BarChart3, AlertTriangle } from 'lucide-react'
import { useLista } from '../../hooks/useLista'

const SERVICIOS_FALLBACK = ['UCI']
...
const CRITERIOS = [
  ...
  { name: 'criterio_4_lista_chequeo',        label: 'Lista de chequeo pacientes con ventilación mecánica' },
  { name: 'criterio_6_interrupcion_sedacion', label: 'Evaluación de posibilidad de VM / interrupción diaria de sedación' },
]
...
const schema = z.object({
  ...
  criterio_4_lista_chequeo:          z.enum(['SI', 'NO', 'NA'], { errorMap: () => ({ message: 'Requerido' }) }),
  criterio_5_presion_neumotaponador: z.coerce.number({ invalid_type_error: 'Requerido' }).min(0).max(60),
  criterio_6_interrupcion_sedacion:  z.enum(['SI', 'NO', 'NA'], { errorMap: () => ({ message: 'Requerido' }) }),
...
export default function AdherenciaPrevencionNavForm() {
  const { id }   = useParams()
  ...
  const servicios = useLista('adherencia_nav_servicio', SERVICIOS_FALLBACK)
  const presion = watch('criterio_5_presion_neumotaponador')
  const presionNum = presion === '' || presion === undefined ? null : Number(presion)
  const fueraDeRango = presionNum != null && !Number.isNaN(presionNum) && (presionNum < 22 || presionNum > 30)

  useEffect(() => {
    if (isEdit) {
      supabase.from('encuesta_adherencia_prevencion_nav').select('*').eq('id', id).single()
        .then(({ data }) => {
          if (data) {
            reset(data)
  ...
  async function onSubmit(values) {
    ...
    const { error } = await guardarEncuesta('encuesta_adherencia_prevencion_nav', payload, isEdit ? id : undefined)
    if (error) {
      setSaveError(error)
      setSaving(false)
      return
    }
    navigate('/encuestas/adherencia-prevencion-nav')
  }
  ...
  if (!isEdit && !puedeCapturar) {
    return <SinPermisoCaptura volverTo="/encuestas/adherencia-prevencion-nav" />
  }
    ...
    <div className="p-6 lg:p-8 animate-fade-in max-w-2xl">
      <div className="flex items-start gap-3 mb-6">
        ...
        <div className="flex-1">
          <h1 className="page-title">{isEdit ? 'Editar' : 'Nuevo'} Registro — Adherencia Prevención NAV</h1>
          <p className="page-subtitle">Neumonía asociada a ventilación mecánica</p>
        </div>
        <Link to="/encuestas/adherencia-prevencion-nav/dashboard"
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="card p-5">
          ...
          <div className="space-y-3">
            ...
            <div className={`p-3 rounded-lg border ${fueraDeRango ? 'border-red-300 bg-red-50' : 'border-slate-200'}`}>
              <p className="text-sm text-slate-700 mb-1">
                Criterio 5 — Presión del neumotaponador (mmHg)
              </p>
              <p className="text-xs text-slate-500 mb-2">
                Rango aceptable: <strong>22 a 30</strong>. Valores fuera de este rango se resaltan en rojo y
                cuentan como hallazgo (=1) en el dashboard.
              </p>
              <input type="number" step="1" className={`input sm:max-w-[160px] ${fueraDeRango ? 'border-red-400 text-red-700 font-semibold' : ''}`}
                {...register('criterio_5_presion_neumotaponador')} />
              {fueraDeRango && (
                <p className="text-xs text-red-600 mt-1.5 flex items-center gap-1">
                  <AlertTriangle className="w-3.5 h-3.5" /> Fuera del rango 22–30
                </p>
              )}
              {errors.criterio_5_presion_neumotaponador && (
                <p className="text-xs text-red-600 mt-1">{errors.criterio_5_presion_neumotaponador.message}</p>
              )}
            </div>

            <RadioSiNoNa name="criterio_6_interrupcion_sedacion" label={CRITERIOS[4].label}
              register={register} error={errors.criterio_6_interrupcion_sedacion} disabled={soloLectura}
              selected={watch('criterio_6_interrupcion_sedacion')} />
          </div>
        </div>
        ...
        <div className="card p-5">
          ...
          <FileUpload value={adjuntos} onChange={setAdjuntos} folder="adherencia-prevencion-nav" />
        </div>
        ...
        <div className="flex items-center justify-end gap-3">
          <Link to="/encuestas/adherencia-prevencion-nav" className="btn-secondary">Cancelar</Link>
